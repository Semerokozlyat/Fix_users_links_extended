...
O365_APS_TYPE_TENANT = "http://www.parallels.com/Office365/Tenant/1.6"
...
def findAllAffectedUsers (api, aps_api, Token, appInstanceId):
    ...
    tenantsInstanceTotalCount = countInstanceResources(api, aps_api, Token, appInstanceId, O365_APS_TYPE_TENANT)
    ...
    allInstanceTenantsMap = {}
    
    path = "aps/2/resources/?implementing(%s)" \
           ",and(eq(aps.status,aps:ready))" \
           ",select(aps.id,subscription.subscriptionId)" \
           ",limit(0,%d)" \
    % (O365_APS_TYPE_TENANT, tenantsInstanceTotalCount)
    allInstanceTenants = aps_api.GET(path, Token)

    # Creating allInstanceTenantsMap dictionary
    for tenant in allInstanceTenants:
        try:
                allInstanceTenantsMap[str(tenant['aps']['id'])] = str(tenant['subscription']['subscriptionId'])
        except Exception as ex:
                log("Tenant with UID" + tenant['aps']['id'] + " does not contain some required fields.", logging.INFO, True)
                log(str(ex), logging.INFO, True)
                pass
    ...
    for user in allInstanceUsers:
        ...
        if allInstanceDomainsMap[str(user['domain']['aps']['id'])] == userMetaData:
            ...
            if userMetaData not in allInstanceDomainsMap.values():
                ...
                affectedUsers[str(user['aps']['id'])] = str("NO such domain: " + userMetaData.split('_')[0] + " with Tenant UID: " + userMetaData.split('_')[1] + " under subscription #" + allInstanceTenantsMap[userMetaData.split('_')[1]])
            else:
				...
				log("Domain with UID" + key + " status is not consistent.", logging.INFO, True)
    return affectedUsers
